...
def load_data(database_filepath):
    ...
    df = pd.read_sql_table("moviesplotly", con=engine)
    # X = df['message']
    # Y = df.iloc[:, 4:]
    Y = df[['revenue']]
    X = df.iloc[:, :-1]
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        print('Please provide the filepath of the movies credites database '\
              'as the first argument and the filepath of the pickle file to '\
              'save the model to as the second argument. \n\nExample: python '\
              'train_classifier.py ../data/InsertDatabaseNamemovies_plotly.db classifier.pkl')
